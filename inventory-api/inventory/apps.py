...
class InventoryConfig(AppConfig):
    ...
    def ready(self):
        ...
        try:
            ...
            logger.info("Inventory ABAC policies registered successfully")
        except Exception as e:
            logger.error(f"Failed to register inventory policies: {e}")
    ...
    def _register_manifest(self):
        ...
        try:
            from .manifest import INVENTORY_SERVICE_MANIFEST
            ...
            response = requests.post(
                ...
                json=INVENTORY_SERVICE_MANIFEST,
                headers={"Content-Type": "application/json"},
                timeout=10
            )
            
            if response.status_code == 200:
                logger.info(f"Successfully registered inventory service manifest: {response.json()}")
            else:
